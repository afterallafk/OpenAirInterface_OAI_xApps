...
# -----------------------------------------------------------------------
# FIX 1 — tbs_per_rb must include NUM_LAYERS (2×2 MIMO).
# The original code used  se * 12 * 14  (1-layer only), causing a 2×
# underestimate of served_bits, reward Mbps, and avg_throughput EMA.
# This is now the single source of truth used by reward, alloc, and EMA.
# -----------------------------------------------------------------------
...
STATE_DIM       = NUM_UES * 5
# Action: per UE → {0=reduce 10%, 1=keep, 2=increase 10%}
...
# -----------------------------------------------------------------------
# FIX 2 — URLLC minimum RB floor.
# The original code let rb_fractions decay to 0.02, which for 106 RBs
# gives int(0.02 * 106) = 2 RBs — starvation under eMBB pressure.
# URLLC UEs are now guaranteed a minimum fraction that covers at least
# MIN_URLLC_RBS resource blocks regardless of the DQN action trajectory.
# -----------------------------------------------------------------------
MIN_URLLC_RBS = 4          # absolute minimum guaranteed to each URLLC UE
MIN_RB_FRAC   = {          # per-type minimum fraction of TOTAL_RBS
...
class DQNAgent:
    ...
    # ------------------------------------------------------------------
    # Reward function — aligned with DICCT 2025 paper objectives:
    #   • Penalise latency violations (especially URLLC)
    #   • Reward normalised throughput per UE type
    #   • Bonus for efficient RB utilisation (~80% target)
    #
    # FIX 3 — Four problems corrected:
    #
    # (a) The original code replaced the URLLC violation penalty with
    #     reward += 0.0, eliminating ALL negative signal.  The DQN then
    #     learned that violations and good service are equivalent (late-
    #     stage violation avg_reward ≈ non-violation avg_reward in CSV).
    #     → Restored a scaled negative penalty proportional to how far
    #       the deadline was exceeded, so the agent can distinguish
    #       severity.  Penalty is capped at -5 per UE to prevent the
    #       gradient explosion that triggered the original "death spiral".
    #
    # (b) The original max(0.0, reward) hard clamp discarded ALL negative
    #     information before it reached the replay buffer.  Bellman
    #     updates then saw only non-negative TD targets, so the Q-values
    #     could never represent "this action is bad" — driving the policy
    #     toward eMBB (large positive tput) and ignoring URLLC.
    #     → Removed the final clamp entirely.  The gradient-clip in
    #       _learn() (norm ≤ 1.0) already prevents instability.
    #
    # (c) Throughput term used raw served_bits / 1e6 / 0.0005 (Mbps),
    #     which is ~2000× larger for eMBB than URLLC.  This overwhelmed
    #     the URLLC penalty and caused eMBB bias even without clamping.
    #     → Normalised per-UE throughput to [0, 1] relative to the
    #       theoretical peak for that UE's current CQI, so URLLC and
    #       eMBB contribute equally to the reward when both are served.
    #
    # (d) URLLC missed packets: paper §IV requires zero effective
    #     throughput on deadline violation.  This is now applied in the
    #     reward (tput_norm = 0) AND in the EMA (eff_tbs = 0) — the same
    #     logic used in LAPF_Enhanced.py.
    # ------------------------------------------------------------------
    def _reward(self, ue_states, rnti_order, allocations):
        ...
        for rnti in rnti_order[:NUM_UES]:
            ...
            # FIX 3a — normalise throughput so all traffic types contribute equally.
            # Peak bits/slot for this UE = tbs_per_rb(se) × TOTAL_RBS
            se        = st["se"]
            ...
            if st["type"] == "URLLC":
                if lat_ms > st["pdb_ms"]:
                    # FIX 3a/3d — scaled negative penalty, cap at -5 per UE.
                    # Proportional to exceedance so agent can learn severity.
                    overshoot = (lat_ms - st["pdb_ms"]) / st["pdb_ms"]
                    reward   -= min(5.0 * overshoot, 5.0)
                    # Deadline missed → zero throughput contribution
                else:
                    # Margin bonus: reward staying well inside the deadline
                    margin  = (st["pdb_ms"] - lat_ms) / st["pdb_ms"]
                    reward += tput_norm * (1.0 + margin)
            else:
                # eMBB / mMTC: penalise excessive latency relative to their PDB
                # FIX 3b — replaced max(0, tput - lat_ratio) with a signed term
                # so the agent learns to avoid extreme queueing on eMBB/mMTC too.
                lat_ratio = lat_ms / max(st["pdb_ms"], 1e-3)
                reward   += tput_norm - min(lat_ratio, 1.0)

        # Resource efficiency bonus: peaks at ~80 % utilisation (unchanged)
        util    = total_rbs_used / max(TOTAL_RBS, 1)
        reward += 2.0 * (1.0 - abs(util - 0.80) / 0.80)

        # FIX 3b — NO final clamp.  Negative rewards must reach the replay
        # buffer so the Bellman update can assign negative Q-values to bad
        # (rnti, action) pairs.  Gradient clipping in _learn() handles stability.
        return float(reward)
    ...
    #
    # FIX 2 — URLLC minimum RB floor enforced here.
    # FIX 1 — served_bits now uses tbs_per_rb() which includes NUM_LAYERS.
    # ------------------------------------------------------------------
    def _actions_to_alloc(self, actions, rnti_order, ue_states):
        ...
        for idx, rnti in enumerate(rnti_order[:NUM_UES]):
            ue_type = ue_states[rnti]["type"]
            delta   = RB_DELTA_FRAC[actions[idx]]
            # FIX 2 — clamp lower bound to per-type minimum fraction
            lo = MIN_RB_FRAC.get(ue_type, 0.02)
            self.rb_fractions[rnti] = max(lo, min(0.80,
                                          self.rb_fractions[rnti] + delta))

        # Renormalise so sum ≤ 1.0 while respecting per-type floors
        total = sum(self.rb_fractions[r] for r in rnti_order[:NUM_UES])
        if total > 1.0:
            # Scale down non-URLLC types first to protect URLLC minimum
            excess   = total - 1.0
            scalable = sum(self.rb_fractions[r]
                           for r in rnti_order[:NUM_UES]
                           if ue_states[r]["type"] != "URLLC")
            if scalable > 0:
                scale = max(0.0, (scalable - excess) / scalable)
                for rnti in rnti_order[:NUM_UES]:
                    if ue_states[rnti]["type"] != "URLLC":
                        lo = MIN_RB_FRAC.get(ue_states[rnti]["type"], 0.02)
                        self.rb_fractions[rnti] = max(lo,
                                                      self.rb_fractions[rnti] * scale)
            else:
                # All UEs are URLLC — simple proportional scale
                for rnti in rnti_order[:NUM_UES]:
        ...
        for rnti in rnti_order[:NUM_UES]:
            if ue_states[rnti]["buffer_bytes"] > 0 and remaining > 0:
                ue_type = ue_states[rnti]["type"]
                # FIX 2 — guarantee at least MIN_URLLC_RBS for URLLC
                min_rbs = MIN_URLLC_RBS if ue_type == "URLLC" else 1
                rbs     = max(min_rbs,
                              min(int(self.rb_fractions[rnti] * TOTAL_RBS),
                                  remaining))
                se      = ue_states[rnti]["se"]
                # FIX 1 — tbs_per_rb includes NUM_LAYERS=2
                alloc[rnti] = {"rbs": rbs,
                                "served_bits": rbs * tbs_per_rb(se)}
                remaining  -= rbs
            else:
    ...
    def handle(self, ind):
        ...
        try:
            ...
            for rnti in active_ues:
                ...
                # URLLC deadline penalty: zero effective throughput on miss
                # (consistent with LAPF_Enhanced.py §4 and paper §IV)
                eff_tbs = 0.0 if (st["type"] == "URLLC" and lat_ms > st["pdb_ms"]) else raw_tbs

                # FIX 1 — tbs_per_rb already includes NUM_LAYERS; Throughput_Mbps
                # is now physically correct (bits / 1e6 / slot_dur_s)
                tput = (eff_tbs / 1e6) / SLOT_DUR_S

                # EMA applied to ALL active UEs (including 0-RB slots)
...
print(f"\n{'='*60}")
print(" DQN Scheduler xApp (DICCT 2025) — Fixed")
...
try:
    ...
    _shutdown.set()          # callbacks stop accepting new ticks immediately
    time.sleep(0.5)          # let any in-flight _learn() finish
finally:
    for i in range(len(mac_hndlr)):
        ric.rm_report_mac_sm(mac_hndlr[i])   # now safe — callback is idle
        ric.rm_report_rlc_sm(rlc_hndlr[i])
    if HAS_TORCH and not _shutdown.is_set():
