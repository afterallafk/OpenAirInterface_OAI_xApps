...
echo "Auto-detecting 8 5G UE IP addresses..."

get_ip() {
    ip -4 addr show "$1" 2>/dev/null | grep -oP '(?<=inet\s)\d+(\.\d+){3}'
}

# 8 UE setup tracking (4 URLLC, 2 eMBB, 2 mMTC)
...
# Validate detection
if [ -z "$IP_URLLC_4" ]; then
    echo "[ERROR] Could not detect all 8 UEs. Ensure oaitun_ue_1 through oaitun_ue_8 are active."
    exit 1
fi
...
echo "Traffic flowing to all 8 UEs! Press Ctrl+C to stop."
