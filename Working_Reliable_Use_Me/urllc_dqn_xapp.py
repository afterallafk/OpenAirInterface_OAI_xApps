...
# =============================================================================
# QoS MAPPING  –  fixed 8-UE topology, matches traffic_gen8.sh
# =============================================================================
QOS_MAPPING = {
...
def compute_reward(latency_ms: float, deadline_ms: float,
    ...
    """
    0.5 × latency_reward  =  1 − clip(latency/deadline, 0, 2)
    0.3 × harq_reward     =  1 − dl_bler
    0.2 × eff_reward      =  clip(served_bits / (RBs × 168 RE × 8), 0, 1)
    −2.0 URLLC miss penalty
    """
    latency_reward = 1.0 - np.clip(latency_ms / max(deadline_ms, EPSILON_FLOAT), 0.0, 2.0)
    harq_reward    = harq_rate
    eff_reward     = np.clip(served_bits / (rbs_granted * 168 * 8 + 1), 0.0, 1.0)
    urllc_penalty  = -2.0 if (traffic_type == "URLLC" and latency_ms > deadline_ms) else 0.0
    return float(0.5 * latency_reward + 0.3 * harq_reward + 0.2 * eff_reward + urllc_penalty)
        ...
        def handle(self, ind):
            try:
                ...
                for ue in ind.ue_stats:
                    ...
                    # wb_cqi: stale-hold + MCS→CQI fallback (verbatim from LAPF)
                    try:
                ...
                for rnti in active_ues:
                    ...
                    mac = _build_mac_from_state(ue, st)   # helper below
                    ...
                    harq_rtt  = mac.dl_num_harq * SLOT_DURATION_MS * 8.0
                    ...
                    # Experience replay + Bellman update
                ...
                # ── 3. Write CSV batch — same pattern as LAPF ─────────────────
                if log_rows:
                    with open(global_ue_state.csv_file, mode="a", newline="") as f:
                        writer = csv.writer(f)
                        writer.writerows(log_rows)

                # ── 4. Heartbeat every ~1 s (100 callbacks × 10 ms = 1 s) ──────
                self._tti += 1
                if self._tti % 100 == 0:
